...
| 5 | `address` | text | UQ | phone_number_id, Page id, IG account, from-address — the API handle the adapter dials, and inbound’s which-merchant lookup: Meta’s webhook says only "a message arrived on phone_number_id 1067…", and this unique index answers whose it is. The human-readable number lives in capabilities. **As built (#1037, ruled 1 Sep 2026): TWO uniques** — `(merchant_id, channel, address)` for the tenant's own rows, plus a GLOBAL partial `(channel, address) WHERE status <> 'retired'` — the second sealed exception to merchant-first (after T16's wamid), earned the same way: an address is the PROVIDER's identifier, and inbound routing works backwards through it, so two merchants holding one live number would make the sender unknowable. Scoped to non-retired rows so a recycled number can live again under its new holder while the tombstone keeps answering history |
...
| 14 | `provider_message_id` | text | UQ | How an inbound receipt finds this row — the wamid; the whole metrics chain hangs on this UNIQUE Trail (11 Sep 2026, #1140): the same UNIQUE answers a SECOND question — a customer's reply carries the provider's id of the message it answers, so `replied_to` → this index → cols 7/8 names the RUN that sent it. A reply is a receipt of another kind, and that is why a listening square needs no `match` and no correlate is ever planted |
