...
| [0022](0022-no-never-words-on-external-surfaces.md) | No never-words (CRM/CDP/CPaaS) on external surfaces: `/crm` prefix dropped → `/ingest/events`, `/customers/*`; tags neutral; internal `app/crm`/`crm_*` stay | Accepted |
| [0023](0023-version-pinning.md) | Runs execute the definition version they entered under: `crm_workflow_version` (T25), `on_publish: pin \| migrate`, one consumer with two reads, versions never deleted, template-retirement guard under an advisory lock — reverses 057's "never an execution pin" | Accepted (built 3 Sep 2026; authored in-repo, mirrored) |
