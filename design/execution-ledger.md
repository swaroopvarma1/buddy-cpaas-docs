...
- **#1038 lands as THREE PRs (ruled 2 Sep 2026, plan posted on the PR)**: **A**
  ...
  **ZERO sync code** — the timer, the route, the seed, the sync/resume/list queries
  ...
  not releasing", so no bridge is needed); **C** webhooks — record's Meta ingress
  bay (`INGRESS` first entry: hub.challenge, X-Hub-Signature-256, merchant stamped
  from the receiving WABA, topics `template.status` / `template.category` /
  `template.quality`) + connectivity's consumer (one `register_consumer` line;
